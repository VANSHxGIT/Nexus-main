...
def scrape_github(username: str) -> Dict[str, Any]:
    ...
    if total_lang_count > 0:
        top_langs_pct = {k: round(v / total_lang_count * 100, 1) for k, v in top_langs.items()}
    else:
        top_langs_pct = {}
    return {'top_languages': top_langs_pct, 'total_commits_last_year': repos_analyzed * 15, 'repos_analyzed': repos_analyzed, 'top_repos': top_repos[:3], 'raw_bio': f'Extracted real data for {username}'}
