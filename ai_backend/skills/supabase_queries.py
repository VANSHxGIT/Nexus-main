...
def get_active_projects() -> List[Dict[str, Any]]:
    try:
        response = (
            supabase.table("projects").select("*").eq("is_active", True).execute()
        )
...
def save_profile(profile_data: Dict[str, Any]) -> Dict[str, Any]:
    try:
        response = (
            ...
            .upsert(profile_data, on_conflict="github_handle")
            .execute()
        )
...
def get_user_profile_and_matches(github_handle: str) -> Dict[str, Any]:
    try:
        ...
        interviews_resp = (
            supabase.table("interviews").select("*").eq("user_id", user_id).execute()
        )
        return {
            "profile": profile,
            "matches": matches_resp.data,
            "interviews": interviews_resp.data,
        }
    except Exception as e:
