...
async def execute_cortex(user_id: str, profile: SynapseSanitizedProfile) -> CortexMatchEvaluation:
    emit_thought('CORTEX', user_id, 'Initiating bi-directional matchmaking analytics...')
    vacs = db.table('project_vacancies').select('id, required_skills').limit(1).execute()
    if vacs.data:
        ...
        vac = db.table('project_vacancies').insert({'project_id': proj_id, 'role_title': 'Backend AI Engineer', 'required_skills': required_skills}).execute()
    ...
    evaluation = await agent.ai(system=system_prompt, user=user_prompt, schema=CortexMatchEvaluation)
    emit_thought('CORTEX', user_id, f'Match evaluated. Confidence: {evaluation.match_confidence_score * 100:.0f}%')
    try:
        db.table('matches').insert({'user_id': user_id, 'vacancy_id': mock_vacancy_id, 'match_confidence_score': evaluation.match_confidence_score, 'justification': evaluation.justification, 'status': 'pending_validation'}).execute()
        emit_thought('CORTEX', user_id, f'Match stored in Supabase successfully.')
