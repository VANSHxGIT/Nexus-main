...
from ai_backend.schemas import SynapseSanitizedProfile, GithubMetrics, LinkedinHistory
...
async def execute_synapse(user_id: str, github_raw: dict, linkedin_raw: dict, resume_text: str = None) -> SynapseSanitizedProfile:
    ...
    try:
        db.table('profiles').upsert({'user_id': user_id, 'sanitized_skills': sanitized_profile.sanitized_skills, 'experience_summary': sanitized_profile.experience_summary, 'github_metrics': sanitized_profile.github_metrics.model_dump(), 'linkedin_history': sanitized_profile.linkedin_history.model_dump()}).execute()
