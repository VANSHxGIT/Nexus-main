...
async def generate_questions(
    ...
    iq = await agent.ai(
        system=system_prompt, user=user_prompt, schema=InterviewQuestions
    )
...
async def evaluate_answers(
    match_id: str, validation_id: str, questions: list, answers: list
    ...
    evaluation = await agent.ai(
        system=system_prompt, user=user_prompt, schema=ValidationEvaluation
    )
    emit_thought(
        "VALIDATOR",
        match_id,
        f"Validation score computed: {evaluation.validation_score * 100:.0f}%",
    )
    try:
