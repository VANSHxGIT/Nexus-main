...
async def process_user_registration(linkedin_url: str, github_handle: str):
    ...
    await asyncio.to_thread(save_match_result, {'user_id': str(user_id), 'project_id': str(match_result.project_id), 'confidence_score': match_result.confidence_score, 'cortex_justification': match_result.cortex_justification})
    await asyncio.to_thread(save_interview, {'user_id': str(user_id), 'project_id': str(match_result.project_id), 'questions': interview_qs, 'answers': {}})
