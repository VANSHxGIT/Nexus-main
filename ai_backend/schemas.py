...
class GithubMetrics(BaseModel):
    top_languages: dict[str, float] = Field(default_factory=dict, description='Language usage percentages')
    total_commits_last_year: int = Field(default=0)
    repos_analyzed: int = Field(default=0)
    top_repos: List[str] = Field(default_factory=list, description='Top repositories by stars or recent activity')
...
class SynapseSanitizedProfile(BaseModel):
    sanitized_skills: List[str] = Field(..., description='Cleaned, unified list of technical skills')
    experience_summary: str = Field(..., description='A professional, non-fluffed summary of experience')
    github_metrics: GithubMetrics
    linkedin_history: LinkedinHistory

class CortexMatchEvaluation(BaseModel):
    match_confidence_score: float = Field(..., ge=0.0, le=1.0, description='Confidence score between 0.0 and 1.0')
...
class ValidationEvaluation(BaseModel):
    ...
    workspace_links: Optional[dict] = Field(default=None, description='Provisioned workspace links upon success')
