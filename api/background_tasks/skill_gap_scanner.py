            ...
            if gapped_projects and devs:
                for dev in devs:
                    profile = UserProfile(
                        ...
                        linkedin_url=dev.get("linkedin_url"),
                        synthesized_bio=dev.get("synthesized_bio"),
                        primary_language=dev.get("primary_language"),
                        verified_skills=dev.get("verified_skills", []),
                    )
                    match = await perform_deep_match(
                        UUID(dev["id"]), profile, gapped_projects
                    )
                    if match and match.confidence_score > 0.85:
                        logger.info(
                            f"Skill Gap Match Found! Dev {dev['github_handle']} mapped to project {match.project_id}"
                        )
                        await asyncio.to_thread(
                            save_match_result,
                            {
                                ...
                                "cortex_justification": match.cortex_justification,
                            },
                        )
